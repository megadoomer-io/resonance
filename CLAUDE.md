...
- **Genre affinity is a pure primitive (#136)**: `generators/genre.py` (`affinity_score`, count-weighted cosine over `genre_mbid` vectors, each seed L2-normalized before aggregation; returns `None` for no-basis vs `0.0` for mismatch). Every genre consumer plugs into it: the disambiguation picker, and the genre-discovery layer. Affinity overlaps only on the *exact* `genre_mbid` (within-genre centrality), NOT cross-genre adjacency — that needs a genre-similarity model (a later arc).
